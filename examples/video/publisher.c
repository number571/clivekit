...
int main() {
    ...
    FILE *writer_pipe = fopen("writer_pipe.ts", "rb");
    if (writer_pipe == NULL) {
        printf("fopen\n");
        return 3;
    }
    ...
    char buff[BUFF_SIZE];
    while(1) {
        int n = fread(buff, sizeof(char), BUFF_SIZE, writer_pipe);
        int status = clivekit_write_data_to_room(room_desc, CLIVEKIT_DTYPE_VIDEO, buff, n);
        if (status) {
            printf("write failed\n");
            return 3;
        }
